...
async def login(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"لطفاً وارد لینک زیر شو و کد PIN رو بفرست:\n{auth_url}")
...
async def handle_pin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in user_sessions:
        await update.message.reply_text("اول دستور /login رو بزن.")
        return
    ...
    try:
        ...
        for username in usernames:
            ...
            if resp.status_code == 200:
                ...
                logging.warning(f"Error blocking @{username}: {resp.text}")
